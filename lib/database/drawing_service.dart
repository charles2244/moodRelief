...
class DrawingService {
  ...
  Future<void> saveDrawing({
    ...
    try {
      final user = '14'; // Replace with actual user_id logic if needed
      if (user == null) {
  ...
  Future<List<DrawingEntry>> fetchRecentDrawings({required String userId}) async {
    try {
  ...
  Future<List<DrawingEntry>> fetchAllDrawings({required String userId}) async {
    try {
      final response = await Supabase.instance.client
          .from('content')
          .select('id, title, created_at, drawing(image_data, canvas_size)')
